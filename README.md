...
| 3 交织 | 横切需求焊接节点；59 个真实纠缠案例逐条出现（业务树也被卷入） | 红色纠缠线生成，节点长出多义性光环；案例栏下的进度条可拖动播放到任意一条，「⚡ 全展示」一键点亮全部——展示完毕停格，不再轮播 |
| 4 引力 | **本征耦合：有些节点势必靠近、无法远离**（12 对 × 三种长法） | 节点挣脱树形互相吸引，洋红引力束 + 被拉弯的枝干；案例栏逐对点名轮播——被点名的光束增粗增亮、两端节点呼吸、中点浮出名牌，面板分组列表可点选任意一对 |
...
| 9 树之树 | **树也长成树：生长顺序即系统命运**——先有 A 再有 B ≠ 先有 B 再有 A | 森林坍缩成元节点，按「从谁的痛点里长出来」长成元树；两条演化路径可切换重播，「生长速度」滑杆随时调速（默认即快），虚线垂向森林真身 |
...
虚线垂向森林中该树的真身，两条路径可随时切换重播，生长速度按倍速播放头累加（中途调速不跳变）；
