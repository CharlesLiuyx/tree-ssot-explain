...
每一步维护一棵 undo-tree 式「视口历史树」（分支不丢失）。右侧「视口历史」默认收起为胶囊，
点击展开后可逐项跳转、「⌫ 清空」一键重置（只留起始视角并飞回）；
**V 键 /「⛶ 全景」** 打开全景总览一屏铺开整棵树；视口超过 30 个时自动垃圾回收——
摘掉最久未访问的旁支叶子，根与当前路径永不回收。

总览步骤默认自动旋转，**Space** 或左下角按钮可随时暂停 / 恢复。
快捷键按**作用域**分发（声明式 KEYMAP，逐域匹配）：全景 overlay 打开时独占键盘
（Esc / V 只负责关闭全景，←/→ 不会在模态背后切步骤）；焦点在输入控件或按钮上时按键交还原生语义。
