...
- **青色幽灵根（图外真相）**：有些节点的真正 SSOT 根本不在仓库里——DI 容器的运行时装配、
  事件总线的订阅表、用户磁盘上的旧文件、部署系统的环境配置、环外依赖你一切可观察行为的
  下游调用方（Hyrum 定律）。图内多义让 AI 改错；
  图外真相让 AI **自信地把文本改对、坏掉的是世界**。唯一出路是「显式化」收编。
- **树之树（元树）**：十六棵树本身也组成一棵更大的树——每棵横切树都是从另一棵树的
  痛点里长出来的。**同一组树，七种生长顺序，长成七棵完全不同的元树**。四条行业原型：
  ...
  再加**开发者积木族**三条真实产品的一生（Terraform / Playwright / Sentry：第五个元根
  「对外契约」的三副面孔——声明 / 承诺 / 口径，它们的单向门被 Hyrum 定律焊在环外的
  下游仓库里——动根者失其树）。
...
| 9 树之树 | **树也长成树：生长顺序即系统命运**——先有 A 再有 B ≠ 先有 B 再有 A | 森林坍缩成元节点，点「▶ 生长」按「从谁的痛点里长出来」长成元树（不点不播，长完自动休眠）；七条演化路径（工具 / SaaS / 企业 / 消费 + 积木族三条：Terraform / Playwright / Sentry）可切换重播，「生长速度」滑杆随时调速（默认即快），虚线垂向森林真身 |

**名词解释**：面板、案例栏、图例中的术语（SSOT、横切、本征耦合、CRDT、幂等、被遗忘权、
邻居噪音、attention 预算……共 138 条）自动标注虚线下划线，悬停即弹出通俗解释。
...
- **显式化 Explicitation（−40%）**：把**图外真相**拉回图内——DI 收敛为单一装配文件、
  ...
  不成文的接口行为锁进契约测试（幽灵根被拉进仓库边界，虚线变实线，节点停止闪烁）。
...
- **交互期动态分辨率**:画布手势(拖拽/缩放)期间临时降到 dpr 1.2(Retina 片元省 ~36%),
  松手 + 600ms 惯性尾巴后恢复满分辨率并立即补渲一帧;只认画布上的真实手势——
  切步相机飞行、自动旋转、点面板按钮都保持满清晰度。与永久降档互补,可逆、按手势作用域。
...
虚线垂向森林中该树的真身，七条路径可随时切换重播，生长速度按倍速播放头累加（中途调速不跳变）；
右上角「语义熵」仪表由叙事阶段、已揭示交织数与策略开关实时计算。
