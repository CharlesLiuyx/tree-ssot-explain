...
export function nodeInfoHTML(n) {
  ...
  if (n.gravity && runtime.gravityPull > .4) {
    ...
    h += `<div class="tt-grav">⚡ 本征耦合（${GRAVITY_KINDS[n.gravity.kind]}）：与「${other}」势必靠近</div><ul><li>${n.gravity.why}</li></ul>`;
  }
...
export function gravityInfoHTML(g) {
  ...
  return `<b>${g.aName} ⚡ ${g.bName}</b><span class="tt-kind">本征耦合 · ${GRAVITY_KINDS[g.kind]}</span>` +
